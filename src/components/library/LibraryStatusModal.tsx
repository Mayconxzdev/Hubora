...
export function LibraryStatusModal({ item, isOpen, onClose }: LibraryStatusModalProps) {
  ...
  const progressFields = () => {
    ...
    if (item.mediaType === 'book') return <div className="grid grid-cols-2 gap-3"><Field label="Página atual"><Input type="number" min={0} value={progress.currentPage || ''} onChange={(e) => updateProgress('currentPage', Number(e.target.value))} /></Field><Field label="Total de páginas"><Input type="number" min={0} value={progress.totalPages || item.pages || ''} onChange={(e) => updateProgress('totalPages', Number(e.target.value))} /></Field></div>;
