  ...
  const analyzeVideo = async (selected: File) => {
    ...
    for (let index = 0; index < frames.length; index += 1) {
      ...
      for (const candidate of frameCandidates) {
        ...
        if (!previous || (candidate.confidence || 0) > (previous.confidence || 0)) merged.set(key, { ...candidate, reason: `${candidate.reason} Evidência encontrada em ${index + 1} de ${frames.length} quadros analisados.` });
      }
    }
    setExtractedText(Array.from(new Set(texts)).join(' • '));
    setResults(Array.from(merged.values()).sort((a, b) => (b.confidence || 0) - (a.confidence || 0)).slice(0, 8));
  ...
  const run = async () => {
    ...
    try {
      if (mode === 'image') {
        ...
        setAnalysisSummary('Descrição comparada textualmente com os catálogos disponíveis; o Radar não atribui compreensão semântica de cenas sem uma fonte que a comprove.');
      } else {
