...
import { analyzeImage, resolveSharedLink, searchSceneDescription, type RadarCandidate } from '@/services/radarSearch';
...
export function Radar() {
  ...
  const analyzeVideo = async (selected: File) => {
    ...
    for (let index = 0; index < frames.length; index += 1) {
      const base = 0.12 + (index / frames.length) * 0.78;
      const response = await analyzeImage(frames[index], {
        allowRemoteAnimeSearch: index === 1 && remoteRadarAvailable && allowRemoteAnimeSearch,
        onProgress: (nextPhase, value) => { setPhase(`Quadro ${index + 1}/${frames.length}: ${nextPhase}`); setProgress(base + value * (0.72 / frames.length)); },
      });
      if (response.extractedText.trim()) texts.push(response.extractedText.trim());
  ...
  const run = async () => {
    ...
    try {
      ...
      setProgress(1);
    } catch (error) { toast.error(error instanceof Error ? error.message : 'Não foi possível pesquisar.'); }
