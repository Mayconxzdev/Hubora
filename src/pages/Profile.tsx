...
export function Profile() {
  ...
  const stats = useMemo(() => {
    ...
    const minutes = library.reduce<number>((total, entry) => {
      if (entry.mediaType === 'movie' && entry.media.runtime && (entry.status === 'completed' || entry.progress.watched)) return total + entry.media.runtime;
    ...
    const topGenres = (Object.entries(genres) as Array<[string, number]>).sort((a, b) => b[1] - a[1]).slice(0, 5);
    return { completed, consuming, favorites, hours: Math.round(minutes / 60), topGenres };
  }, [library]);

  if (!user) return <div className="hub-page"><SEO title="Minha conta"/><header className="hub-page-header items-start"><div><div className="hub-section-eyebrow"><User size={14}/> Conta privada</div><h1 className="hub-page-title">Minha conta</h1><p className="hub-page-subtitle">Seu perfil aparece aqui quando você está autenticado.</p></div></header><div className="hub-empty-state"><User size={34}/><p>Entre na sua conta para acessar a sincronização pessoal.</p><Link to="/login" className="hub-btn hub-btn-primary mt-4 px-6 text-sm font-bold" style={{ minHeight: '44px' }}>Entrar</Link></div></div>;
  ...
  return <div className="hub-page mx-auto max-w-5xl">
    ...
    <section className="hub-section"><div className="grid grid-cols-2 gap-3 lg:grid-cols-5">{([
      ['Biblioteca', library.length, Library], ['Em andamento', stats.consuming, Clock3], ['Concluídos', stats.completed, CheckCircle2], ['Favoritos', stats.favorites, Heart], ['Horas registradas', stats.hours, BarChart3],
