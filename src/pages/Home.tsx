...
import { CATEGORY_NAVIGATION as CATEGORIES } from '@/config/navigation';
...
function reasonFor(mode: DecisionMode, item: MediaItem, continuing: boolean) {
  if (mode === 'continue' && continuing)
    return 'Você já começou esta obra. É o caminho mais curto entre abrir o Hubora e voltar para a história.';
  if (mode === 'short') {
    if (item.mediaType === 'tv' || item.mediaType === 'anime')
      return 'Um episódio cabe melhor numa janela curta e mantém seu progresso fácil de retomar.';
    if (item.runtime)
      return `A duração de aproximadamente ${item.runtime} minutos combina melhor com o tempo que você tem agora.`;
...
export function Home() {
  ...
  const continueItems = useMemo(
    () => library.filter((item) => item.status === 'consuming').sort((a, b) => b.lastInteractedAt - a.lastInteractedAt),
    [library],
  );
  const planningItems = useMemo(
    () => library.filter((item) => item.status === 'planning').sort((a, b) => b.lastInteractedAt - a.lastInteractedAt),
    [library],
  );
  ...
  const candidates = useMemo(() => {
    ...
    if (mode === 'short') {
      const localShorts = library
        ...
        .filter(
          (m) => ['anime', 'manga', 'comic', 'book', 'novel'].includes(m.mediaType) || (m.runtime && m.runtime <= 45),
        );

      const feedShorts = [...feed].filter(
        (m) => ['anime', 'manga', 'comic'].includes(m.mediaType) || (m.runtime && m.runtime <= 110),
      );
  ...
  const continuing = Boolean(
    decisionItem && continueItems.some((entry) => itemKey(entry.media) === itemKey(decisionItem)),
  );
  ...
  return (
    <div className="hub-page hub-home-page hub-home-v9">
      <SEO
        title="Início"
        description="Escolha o que combina com seu momento e encontre a melhor fonte para consumir."
      />
      ...
      <div className="relative z-10 space-y-12">
        ...
        {continueItems.length > 0 && (
          <section className="hub-section space-y-4">
            ...
            <div className="grid auto-cols-[min(88vw,22rem)] grid-flow-col gap-4 overflow-x-auto pb-4 scrollbar-hide sm:auto-cols-[20rem] lg:auto-cols-[22rem]">
              {continueItems.slice(0, 8).map((entry) => (
        ...
        {/* Descoberta guiada: principal no onboarding, secundária quando já há progresso */}
        <section className={cn('text-center max-w-2xl mx-auto space-y-3', continueItems.length ? 'py-2' : 'py-8')}>
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[var(--hub-brand-soft)] text-[var(--hub-brand)] text-xs font-bold tracking-wide uppercase">
            <Sparkles size={12} /> Escolha guiada
          </span>
          ...
          <p className="text-sm text-slate-400 font-medium">
            Escolha seu momento. O Hubora compara dados reais da sua biblioteca e dos catálogos disponíveis.
          </p>
        </section>
        ...
        <section className="hub-recommendation-panel" aria-live="polite">
          <div className="hub-recommendation-art">
            {decisionItem && (decisionItem.backdropPath || decisionItem.posterPath) ? (
              ...
              <div className="grid place-items-center h-full text-slate-600 bg-slate-900/20">
                <Sparkles size={38} />
              </div>
            )}
          </div>
          <div className="hub-recommendation-copy">
            ...
            {decisionItem && (
              <p className="hub-recommendation-meta">
                {decisionItem.mediaType === 'tv'
                  ? 'Série'
                  : decisionItem.mediaType === 'movie'
                    ? 'Filme'
                    : decisionItem.mediaType === 'comic'
                      ? 'Quadrinho'
                      : decisionItem.mediaType === 'book'
                        ? 'Livro'
                        : decisionItem.mediaType === 'novel'
                          ? 'Novel'
                          : decisionItem.mediaType === 'game'
                            ? 'Jogo'
                            : decisionItem.mediaType === 'manga'
                              ? 'Mangá'
                              : 'Anime'}
                {decisionItem.releaseDate ? ` • ${decisionItem.releaseDate.slice(0, 4)}` : ''}
                {decisionItem.voteAverage ? ` • Nota ${decisionItem.voteAverage.toFixed(1)}` : ''}
              </p>
            )}
            <div className="hub-recommendation-reason">
              ...
              <p>
                {decisionItem
                  ? reasonFor(mode, decisionItem, continuing)
                  : 'O assistente está analisando seu catálogo...'}
              </p>
            </div>
            <div className="hub-recommendation-actions">
              <Button
                ...
                disabled={!decisionItem}
                className="bg-[var(--hub-brand)] hover:bg-[var(--hub-brand-strong)] text-black font-bold"
              >
                <Play size={16} fill="currentColor" className="mr-2" />{' '}
                {continuing ? 'Abrir para continuar' : 'Ver detalhes'}
              </Button>
              <Button
                size="lg"
                variant="outline"
        ...
        <section className="space-y-4">
          <h3 className="text-xs font-black uppercase tracking-widest text-[var(--hub-muted)]">
        ...
        {library.length > 0 && (
          <section className="hub-section space-y-4">
            <div className="hub-section-heading flex justify-between items-end">
              <div>
                <div className="hub-section-eyebrow text-xs uppercase font-bold text-[var(--hub-muted)] mb-1 flex items-center gap-1.5">
                  <Zap size={13} /> Novidades na lista
                </div>
                <h2 className="hub-section-title text-xl font-extrabold text-white">Recentemente adicionados</h2>
              </div>
              <Button
                variant="ghost"
                size="sm"
                ...
                className="text-xs font-bold hover:bg-white/5"
              >
                Ver biblioteca <ArrowRight size={14} className="ml-1" />
              </Button>
            </div>
            <div className="grid auto-cols-[min(88vw,22rem)] grid-flow-col gap-4 overflow-x-auto pb-4 scrollbar-hide sm:auto-cols-[20rem] lg:auto-cols-[22rem]">
              {[...library]
        ...
        <section className="hub-home-tools grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button
            ...
            className="flex items-center gap-4 p-5 rounded-2xl border border-white/5 bg-slate-900/30 hover:bg-slate-900/60 hover:border-white/10 text-left transition"
          >
            <span className="p-3 bg-slate-900 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              ...
              <small className="block text-xs text-slate-400 mt-1 leading-normal">
                Consulte capacidades, requisitos e o estado de cada integração.
              </small>
            </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <small className="block text-xs text-slate-400 mt-1 leading-normal">
                Acesso a obras abertas de leitura e catálogo de mídia gratuita.
              </small>
            </div>
            <ArrowRight size={18} className="text-slate-500" />
          </button>
