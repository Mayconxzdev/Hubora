...
test('exporta, valida, restaura, importa CSV, cria snapshot e reseta dados do visitante', async ({ page }) => {
  ...
  // Hades faz parte do catálogo local autorizado de jogos. Assim, esta jornada
  // de backup testa dados reais da biblioteca sem depender de TMDB/Google Books.
  const seedTitle = 'Hades';
  await page.goto('/login');
  await page.getByRole('button', { name: /Entrar como Visitante/i }).click();

  await page.goto('/games');
  await page.getByRole('searchbox', { name: 'Pesquisar nesta categoria' }).fill(seedTitle);
  await page.getByRole('button', { name: `Adicionar ${seedTitle} à biblioteca` }).first().click();
  ...
  expect(exported.library?.some((entry) => entry.title === seedTitle)).toBe(true);
  ...
  await expect(page.getByLabel('Sincronizar PC e Android').getByRole('link', { name: /Entrar para sincronizar/i })).toHaveAttribute('href', '/login');
